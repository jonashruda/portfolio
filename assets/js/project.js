...
function renderProject(p){
  ...
  media.innerHTML = '';
  if(p.videoUrl){ media.innerHTML += `<div class="card"><iframe src="${p.videoUrl}" allowfullscreen loading="lazy"></iframe></div>`; }
  if(p.imageUrls && p.imageUrls.length){ p.imageUrls.forEach(u => media.innerHTML += `<div class="card"><img src="${u}" alt=""></div>`); }
...
fetch('data/projects.json')
  ...
  .then(json => {
    ...
    if(!p){ h1.textContent='Not found'; desc.textContent='Invalid or missing project slug.'; return; }
    renderProject(p);
  })
  .catch(() => { h1.textContent='Error'; desc.textContent='Could not load data/projects.json'; });
