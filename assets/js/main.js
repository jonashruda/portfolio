...
// Hero texts
Promise.all([
  fetch('data/hero_intro.txt').then(r => r.text()).catch(()=> 'Welcome to my portfolio.'),
  fetch('data/hero_title.txt').then(r => r.text()).catch(()=> 'Programming · Video · 3D')
]).then(([intro, title]) => {
  document.getElementById('intro').textContent = intro.trim();
